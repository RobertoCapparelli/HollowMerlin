...
namespace AIV_Metroid_Player {
    public class Player : MonoBehaviour, IDamageble {
        ...
        private void Start() {
            ...
            energyModule.OnEnergyUpdated += OnEnergyUpdated;
            energyModule.InitMe(this);
            playerController.enoughtEnergy += EnoughtEnergy;
            playerController.consumeEnergy += ConsumeEnergy;
            ResetHealth();
        ...
        //private void Update() {
        //    energyModule.OnUpdate(Time.deltaTime);
        //}
        ...
        public bool EnoughtEnergy (float energy) {
            return energyModule.EnoughtEnergy(energy);
        }

        public void ConsumeEnergy (float energy) {
            energyModule.ConsumeEnergy(energy);
        }

        private void OnEnergyUpdated (float energy) {
            GlobalEventManager.CastEvent(GlobalEventIndex.PlayerEnergyUpdated,
                GlobalEventArgsFactory.PlayerEnergyUpdatedFactory(energyModule.MaxEnergy, energy));
        }
        #endregion
