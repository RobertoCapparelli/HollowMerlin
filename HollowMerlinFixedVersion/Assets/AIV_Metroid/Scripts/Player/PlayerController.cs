...
namespace AIV_Metroid_Player {
    public class PlayerController : MonoBehaviour {
        ...
        #region EnergyModule
        public Func<float, bool> enoughtEnergy;
        public Action<float> consumeEnergy;

        public bool EnoughtEnergy (float energy) {
            if (enoughtEnergy != null) {
                return enoughtEnergy.Invoke(energy);
            }
            return false;
        }

        public void ConsumeEnergy (float energy) {
            consumeEnergy?.Invoke(energy);
        }
        #endregion
